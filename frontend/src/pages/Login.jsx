...
function GoogleButton({ onClick, label = "Continue with Google" }) {
  return (
    <button onClick={onClick} data-testid="google-login-btn"
...
export default function Login() {
  ...
  const submit = async (e) => {
    ...
    catch (e2) { setErr(e2?.response?.data?.detail || "Sign-in failed."); }
    finally { setBusy(false); }
  };

  return (
    <div className="mx-auto flex max-w-md flex-col px-5 py-20">
      <p className="label text-orient-deep">Welcome back to</p>
      <h1 className="mt-3 text-4xl tracking-tight sm:text-5xl">
        <RotatingBrand />
      </h1>
      <p className="mt-4 text-sm text-ink-soft">Sign in to join circles and follow your connections.</p>

      <div className="mt-8"><GoogleButton onClick={googleLogin} /></div>
      ...
      <form onSubmit={submit} className="flex flex-col gap-4">
        ...
        <label className="flex flex-col gap-1.5">
          ...
          <div className="relative">
            <input data-testid="login-password" type={showPw ? "text" : "password"} required value={password} onChange={(e) => setPassword(e.target.value)}
